...
void print_help()
{
    ...
    cout << "rm <path>    - Remove a file or directory\n";
...
void list_directory(const fs::path &dir)
{
    ...
    {
        cout << entry.path().filename().string() << (fs::is_directory(entry) ? "\\" : "") << endl;
    }
}

void change_directory(fs::path &current_path, const string &dir)
{
...
void make_directory(const fs::path &current_path, const string &dir)
{
    fs::path new_dir = current_path / dir;
    if (fs::create_directory(new_dir))
    {
        cout << "Directory created: " << new_dir.string() << endl;
    }
    else
    {
...
void remove_path(const fs::path &current_path, const string &path)
{
    fs::path target = current_path / path;
    if (fs::exists(target))
    {
        fs::remove_all(target);
        cout << "Removed: " << target.string() << endl;
    }
    else
    {
...
void create_file(const fs::path &current_path, const string &filename)
{
    fs::path file_path = current_path / filename;
    ofstream file(file_path);
    if (file)
    {
        cout << "File created: " << file_path.string() << endl;
    }
    else
    {
...
int main()
{
    ...
    fs::path current_path = fs::absolute(project_folder);
    fs::create_directory(current_path); // Ensure project folder exists

    cout << "Project Folder: " << current_path.string() << endl;
    print_help();
    string cmd;
    ...
    while (true)
    {
        ...
        if (cmd == "exit")
            ...
            change_directory(current_path, cmd.substr(3));
        else if (cmd.rfind("mkdir ", 0) == 0)
            make_directory(current_path, cmd.substr(6));
        else if (cmd.rfind("rm ", 0) == 0)
            remove_path(current_path, cmd.substr(3));
        else if (cmd.rfind("touch ", 0) == 0)
            create_file(current_path, cmd.substr(6));
        else if (cmd == "pwd")
